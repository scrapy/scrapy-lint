...
def fills_missing_entries(addon, name, key_path):
    """Return whether *addon* only sets entry *key_path* of dict setting
    *name* when that entry is not already there.

    A setting that instead always overwrites the entry is where restating it
    in a project is redundant, since the project value never sticks.
    """
    try:
        ...
        key = key_path
    settings = Settings()
    settings.set(name, {key: SENTINEL}, "project")
    try:
        addon().update_settings(settings)
    except Exception:  # noqa: BLE001
        # Cannot tell, so assume the safer, quieter answer.
        return True
    value = settings[name]
    return hasattr(value, "get") and value.get(key) == SENTINEL
...
def main(paths):
    ...
    defaults = sorted(
        ...
        and value
        and fills_missing_entries(addon, name, next(iter(value)))
    )
    print(
        dumps(
            {
                "values": {
                    k: UNKNOWN if k in unknown else v for k, v in baseline.items()
                },
                "defaults": defaults,
            }
        )
...
def render(addons: dict[AddonSpec, dict[Version, dict]]) -> str:
    ...
    return (
        ...
        "from scrapy_lint.addons import Addon, Default, VersionedSettings\n"
...
def render_changes(changes: dict) -> str:
    defaults = set(changes["defaults"])
    return "".join(
        f"                {dumps(name)}: {render_entry(name, value, defaults)},\n"
        for name, value in sorted(changes["values"].items())
    )


def render_entry(name: str, value, defaults: set[str]) -> str:
    rendered = render_value(value, 4)
    return f"Default({rendered})" if name in defaults else rendered
